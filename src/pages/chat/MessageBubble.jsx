...
export function MessageBubble({ message, onWebSearch, messageIndex }) {
  ...
  const renderMarkdownInline = (text) => {
    ...
    while ((match = linkRegex.exec(text)) !== null) {
      ...
      pieces.push(
        <a key={match.index} href={match[2]} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">
          {match[1]}
    ...
    return lines.map((line, idx) => {
      ...
      if (line.startsWith('- [')) {
        ...
        if (match) {
          ...
          return (
            <div key={idx} className="mb-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded border-l-4 border-blue-400">
              <a href={url} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline font-medium flex items-center gap-1">
                {title}
  ...
  return (
    <div className={cn('flex gap-3', isUser ? 'justify-end' : 'justify-start')}>
      {!isUser && (
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 via-purple-600 to-pink-600 flex items-center justify-center shrink-0 shadow-sm animate-pulse">
      ...
      <div className={cn(
        ...
        isUser
          ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-br-md shadow-sm shadow-blue-500/20'
          : isError
            ? 'bg-red-50 border border-red-200 text-red-900 rounded-bl-md dark:bg-red-900/20 dark:border-red-800 dark:text-red-100'
            : isWebSearch
              ? 'bg-blue-50 border border-blue-200 text-blue-950 rounded-bl-md dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-50'
        ...
        {message.webSearchAnswer && (
          <div className="mt-3 pt-3 border-t border-blue-200 dark:border-blue-800 bg-blue-50/60 dark:bg-blue-950/20 p-3 rounded-lg">
            <div className="flex items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2 text-blue-700 dark:text-blue-300">
                <Globe className="w-4 h-4" />
                <span className="text-xs font-semibold uppercase tracking-wide">Web Insights</span>
              </div>
              <span className="text-[10px] font-bold uppercase tracking-widest rounded-full bg-blue-100/80 dark:bg-blue-900/30 px-2 py-0.5 text-blue-700 dark:text-blue-300">
                Grounded
              </span>
            </div>
            <div className="prose prose-sm max-w-none dark:prose-invert break-words whitespace-normal w-full text-slate-700 dark:text-slate-200">
              {renderMarkdownBlock(message.webSearchAnswer)}
            </div>
            {message.webSearchSources && message.webSearchSources.length > 0 && (
              <div className="pt-3 mt-3 border-t border-blue-200/70 dark:border-blue-800/60">
                ...
                <div className="flex flex-wrap gap-2">
                  {message.webSearchSources.slice(0, 5).map((source, idx) => (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-[11px] text-blue-700 dark:text-blue-300 hover:border-blue-300 dark:hover:border-blue-700 transition-colors shadow-sm"
                    >
