            ...
            filteredFolderFiles.map((file) => (
              <div key={file.id} className="flex items-center gap-4 rounded-2xl border border-border/10 bg-muted/5 p-3 transition-all hover:bg-muted/10 group">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[13px] font-bold text-foreground/90 group-hover:text-emerald-500 transition-colors">{file.filename}</p>
                  <div className="mt-1 flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-[10px] text-muted-foreground font-bold flex items-center gap-1.5">
                      <Network className="w-3 h-3 text-emerald-500/60" /> {file.node_count || 0}
                    </span>
                    <span className="text-[10px] text-muted-foreground font-bold flex items-center gap-1.5">
                      <GitFork className="w-3 h-3 text-emerald-500/60" /> {file.relationship_count || 0}
                    </span>
                  </div>
                </div>
                <Badge variant="outline" className="shrink-0 text-[10px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-lg border-emerald-500/20 text-emerald-500/80">
                  {file.file_type}
                </Badge>
              </div>
            ))
